...
bool Terrain::GenerateTerrain(const string &heightmap,vec3 sizeVertices,vec2 sizeUv)
{
	...
	for (DWORD y=0;y<height-2;y++)
	{
		for (DWORD x=0;x<width-2;x++)
		{
			vec3 n1=-Cross(vertex[currentIndex+2]-vertex[currentIndex],vertex[currentIndex+1]-vertex[currentIndex]).Normalize();
			normal[currentIndex++]=n1;
			normal[currentIndex++]=n1;
			normal[currentIndex++]=n1;
			
			vec3 n2=-Cross(vertex[currentIndex+2]-vertex[currentIndex],vertex[currentIndex+1]-vertex[currentIndex]).Normalize();
			normal[currentIndex++]=n2;
			normal[currentIndex++]=n2;
			normal[currentIndex++]=n2;
		}
	}
	...
	for (DWORD y=0;y<height-2;y++)
	{
		for (DWORD x=0;x<width-2;x++)
		{
			uv[currentIndex++]=vec2((float)x/(float)(width-1)*sizeUv.x,(float)y/(float)(height-1)*sizeUv.y);
			uv[currentIndex++]=vec2((float)x/(float)(width-1)*sizeUv.x,(float)(y+1)/(float)(height-1)*sizeUv.y);
			uv[currentIndex++]=vec2((float)(x+1)/(float)(width-1)*sizeUv.x,(float)y/(float)(height-1)*sizeUv.y);

			uv[currentIndex++]=vec2((float)(x+1)/(float)(width-1)*sizeUv.x,(float)y/(float)(height-1)*sizeUv.y);
			uv[currentIndex++]=vec2((float)x/(float)(width-1)*sizeUv.x,(float)(y+1)/(float)(height-1)*sizeUv.y);
			uv[currentIndex++]=vec2((float)(x+1)/(float)(width-1)*sizeUv.x,(float)(y+1)/(float)(height-1)*sizeUv.y);
		}
	}
	uvBuffer->LoadData(&uv[0],uv.size()*sizeof(vec2),STATIC);
	index.resize((width-1)*(height-1)*2*3);	
