...
GridWindow::GridWindow(FireCube::Engine *engine) : Object(engine)
{

}

void GridWindow::Render(EditorState *editorState, GridDescriptor *descriptor)
{
	...
	countXInput.Render("CountX", editorState, "Change CountX", [descriptor]() {
		return descriptor->GetCountX();
	}, [descriptor, this](unsigned int newValue) {
		descriptor->SetCountX(newValue, engine);
	});

	countZInput.Render("CountZ", editorState, "Change CountZ", [descriptor]() {
		return descriptor->GetCountZ();
	}, [descriptor, this](unsigned int newValue) {
		descriptor->SetCountZ(newValue, engine);
	});
