...
GeometryPtr CollisionShape::GetFirstGeometry(NodePtr node)
{
	...
	{
		GeometryNodePtr geometryNode = dynamic_pointer_cast<GeometryNode>(node);
...
bool CollisionShape::GetPointInfo(vec3 pos, vec3 &dir, float &dist, bool accurateOutsideDist)
{
    ...
    if (interpolateMesh)
    {
        ...
        int ik1 = ik0 + 1;
        if (ii0 < 0) ii0 = 0;
        else if (ii0 >= sizex) ii0 = sizex - 1;
        if (ij0 < 0) ij0 = 0;
        else if (ij0 >= sizey) ij0 = sizey - 1;
        if (ik0 < 0) ik0 = 0;
        else if (ik0 >= sizez) ik0 = sizez - 1;
        if (ii1 < 0) ii1 = 0;
        else if (ii1 >= sizex) ii1 = sizex - 1;
        if (ij1 < 0) ij1 = 0;
        else if (ij1 >= sizey) ij1 = sizey - 1;
        if (ik1 < 0) ik1 = 0;
        else if (ik1 >= sizez) ik1 = sizez - 1;
    ...
    else
    {
        ...
        if (ii < 0) ii = 0;
        else if (ii >= sizex) ii = sizex - 1;
        if (ij < 0) ij = 0;
        else if (ij >= sizey) ij = sizey - 1;
        if (ik < 0) ik = 0;
        else if (ik >= sizez) ik = sizez - 1;
